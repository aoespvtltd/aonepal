...
export default function BlogPosts() {
  ...
  const handleAnchorClick = (event, topic) => {
    event.preventDefault()

    const element = document.getElementById(topic.toLowerCase().replace(/\s+/g, '-'))
    if (element) {
      window.scrollTo({
        top: element.offsetTop - 10 * 16, // Adjusting for the 10rem offset (1rem = 16px)
        behavior: 'smooth'
      })
    }
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <CardContent>
              <ul className="space-y-2 text-sm">
                {currentPost.description
                  .split('**')
                  .filter((item, index) => index % 2 !== 0) // Filter only the topics (odd-index items after split)
                  .map((topic, index) => (
                    <a
                      key={index}
      ...
      <div className="mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {sortedPosts.slice(0,3).sort((a,b)=> a.createdAt - b.createdAt).filter(post => post.slug !== currentPost.slug).map((post) => (
            <Card key={post.slug}>
              <CardHeader>
